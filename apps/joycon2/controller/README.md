...
振動通知はダッシュボードでON/OFFと強さ1〜5を設定できます。初期値はON・強さ5です。Codexの作業完了は長めに2回、承認待ちは長めに3回振動します。通知はMac操作を行わないため、ControllerがDISABLED中でも動作します。初回はCodexで`/hooks`を開き、`Codex Grip Joy-Con haptic notifications`を確認して信頼してください。既存のComputer Use通知設定は変更しません。
