...
普段はApplicationsフォルダの`Codex Grip.app`をダブルクリックするだけで起動できます。すでに起動中の場合は、重ねて起動せずダッシュボードを開きます。
...
デバッグUIの「Button / Mouse settings」では、DISABLED中に音声入力、ボイスモード、送信、DELETE、新規、前面化、Escape・戻る、タスク選択 上／下、推論レベル 上／下、入力欄切替／机上クリック、マウスクリックとマウス操作ON/OFFを変更できます。同じ機能を複数ボタンへ割り当てられます。「バックアップ」でJSONを書き出し、「復元」でボタン・マウス設定を戻せます。復元もDISABLED中だけ可能です。保存後は全ボタンを一度離し、再度`+`を長押しします。`+`短押しはボイスモード、長押しは安全用MASTERです。

バッテリー欄は、電圧から計算した残量目安をメーター表示します。3.55V以下では赤表示になり、接続中1回だけMac通知で充電を促します。表示％は電圧ベースの目安で、Joy-Con本体が返す正確な残量％ではありません。

振動通知はダッシュボードでON/OFFと強さ1〜5を設定できます。初期値はON・強さ5です。Codexの作業完了は長めに1回、たださんの操作が必要な承認待ちは長めに3回振動します。Codexの自動審査で処理される承認要求では振動しません。通知はMac操作を行わないため、ControllerがDISABLED中でも動作します。初回はCodexで`/hooks`を開き、`Codex Grip Joy-Con haptic notifications`を確認して信頼してください。既存のComputer Use通知設定は変更しません。
...
マウス操作ON時は、Joy-Conを机に置くと光学センサー、持ち上げるとStickへ自動で切り替わります。ZRは机上センサーモードでは左クリック、持ち上げ中はmacOSのアクセシビリティ情報でメイン／サイド入力欄を交互に切り替えます。座標や入力本文は使用しません。Stick押込はどちらのモードでも左クリックです。切替直後の値と大きな飛び値は無視します。マウス操作OFF時のみ、Stick左右は従来どおり前後チャット移動です。Stickと机上マウスの速度は、標準の1.00×を基準に最大4.00×まで調整できます。
