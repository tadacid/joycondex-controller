...
普段はApplicationsフォルダの`JoyCondex.app`をダブルクリックするだけで起動できます。すでに起動中の場合は、重ねて起動せずダッシュボードを開きます。
...
振動通知はダッシュボードでON/OFFと強さ1〜5を設定できます。初期値はON・強さ5です。Codexの作業完了は長めに1回、ユーザー操作が必要な承認待ちは長めに3回振動します。Codexの自動審査で処理される承認要求では振動しません。通知はMac操作を行わないため、ControllerがDISABLED中でも動作します。初回はCodexで`/hooks`を開き、JoyCondexの振動通知フックを確認して信頼してください。既存のComputer Use通知設定は変更しません。
