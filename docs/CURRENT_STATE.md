...
- `JoyCondex.app`からの手動起動と、起動済み時のダッシュボード表示
- Node標準テスト
...
`/Applications/JoyCondex.app`からLIVE Controllerを起動し、再クリック時に二重起動せずダッシュボードを表示する設計です。旧名から変更後のアプリ起動は最終確認が必要です。実設定のバックアップJSONをダウンロードできることは確認済みです。
