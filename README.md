...
以前の会話では「Codex Mini Keyboard」「Codex Micro+」「M5 Codex Grip」などの呼び名も使っていますが、このリポジトリでは親プロジェクト名を **Codex Grip** とします。

## Codexで続きを始める

Codexには**この `codex-grip` フォルダを丸ごと開かせてください**。
...
普段は、Applicationsフォルダの`Codex Grip.app`をダブルクリックします。未起動ならLIVEコントローラーを起動し、起動済みならダッシュボードを開きます。

## 現在のJoy-Con 2操作

| Joy-Con 2 | Codex Grip V0 |
|---|---|
...
`http://127.0.0.1:8788/` の設定画面では、DISABLED中にボタン割当、マウス操作ON/OFF、Stick・机上マウス速度、振動通知のON/OFFと強さを変更できます。設定はJSONへバックアップ・復元でき、再起動後も維持されます。Codexの作業完了は長い振動を2回、承認待ちは長い振動を3回送り、通知だけはDISABLED中も動きます。バッテリー欄には電圧から計算した残量目安メーターを表示し、3.55V以下では画面とMac通知で充電を促します。
