...
現在の実装では、作者の自作アプリ「VoiceKey」へ開始・停止命令を送り、Rを押している間だけ録音します。VoiceKeyはこのリポジトリには含まれていません。

別の音声入力を使う場合は、このリポジトリをCodexで開き、次のように依頼してください。

> VoiceKey連携を、私が使っている音声入力アプリの開始・停止方法に置き換えてください。Rを押している間だけ録音する操作は維持してください。

使っている音声入力アプリの名前やショートカットも一緒に伝えると変更しやすくなります。
