...
`http://127.0.0.1:8788/` の設定画面では、DISABLED中にボタン割当、マウス操作ON/OFF、Stick・机上マウス速度を変更できます。設定はJSONへバックアップ・復元でき、再起動後も維持されます。バッテリー欄には電圧から計算した残量目安メーターを表示し、3.55V以下では画面とMac通知で充電を促します。
